def set_webtoon_style():
    ...
    style_prompt = (
        "웹툰 스타일:\n"
        "- 사실적인 한국 웹툰 스타일\n"
        "- 남자: 35세 한국인, 이마를 덮는 직모, 짧은 머리, 흰색 티셔츠 착용\n"
        "- 여자: 30세 한국인, 어깨까지 오는 직모 머리, 주름없는 흰색 원피스 착용\n"
        "- 배경: 스토리에 맞는 현대적이고 아늑한 설정\n"
        "- 이 장면에 다른 캐릭터는 등장하지 않음\n"
        "- 캐릭터들의 얼굴 표정과 감정에 초점을 맞춰 관계의 긴장감과 복잡성을 전달\n"
        "- 반드시 한국 웹툰 스타일로 그릴 것"
    )
